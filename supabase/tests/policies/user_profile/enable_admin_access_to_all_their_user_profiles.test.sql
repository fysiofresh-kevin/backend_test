...
SELECT tests.create_supabase_user('client_3');

-- Create roles
SELECT public.map_role_and_permissions('role_with_admin_permission', ARRAY['organization:read', 'organization:admin']);
...
INSERT INTO public.user_has_role
    (user_id, role)
VALUES
    ((tests.get_supabase_user('user_with_admin_permission') ->> 'id')::uuid, 'role_with_admin_permission'),
...
INSERT INTO public.user_profile
    (user_id, name)
VALUES
    ((tests.get_supabase_user('client_1') ->> 'id')::uuid, 'Client One'),
    ((tests.get_supabase_user('client_2') ->> 'id')::uuid, 'Client Two'),
    ((tests.get_supabase_user('client_3') ->> 'id')::uuid, 'Client Three');
...
SELECT results_eq(
               ...
               $$VALUES (3::bigint)$$,
               'confirm that admin can read all three profiles'
       );
