...
INSERT INTO public.user_has_role
    (user_id, role)
VALUES
    ((tests.get_supabase_user('client_with_appointments') ->> 'id')::uuid, 'role_with_appointment_read_permission'),
    ((tests.get_supabase_user('client_without_appointments') ->> 'id')::uuid, 'role_with_appointment_read_permission'),
    ((tests.get_supabase_user('client_without_permission') ->> 'id')::uuid, 'role_without_permission');


SELECT public.create_appointment(1, (SELECT tests.get_supabase_uid('client_with_appointments')), (SELECT tests.get_supabase_uid('employee')), 'completed');
SELECT public.create_appointment(2, (SELECT tests.get_supabase_uid('client_with_appointments')), (SELECT tests.get_supabase_uid('employee')), 'completed');
SELECT public.create_appointment(3, (SELECT tests.get_supabase_uid('client_with_appointments')), (SELECT tests.get_supabase_uid('employee')), 'completed');
SELECT public.create_appointment(4, (SELECT tests.get_supabase_uid('client_without_permission')), (SELECT tests.get_supabase_uid('employee')), 'completed');
...
SELECT results_eq(
               ...
               $$VALUES (3::bigint)$$,
               'confirm that client can read their three appointments'
       );
