...
SELECT has_function(
               'public',
               'update_client_to_employees_relationships',
               ARRAY ['uuid', 'uuid[]'],
               'Function: update_client_to_employees_relationships exists with uuid arguments');
...
SELECT is(
               update_client_to_employees_relationships(
                       ...
                       ARRAY [
                           ...
                           (tests.get_supabase_user('employee_2') ->> 'id')::UUID
                           ]
               ),
               'ok',
               'Function returns ok'
...
SELECT is(
               (SELECT ARRAY_AGG(employee_id)
                FROM employee_has_clients
                WHERE client_id = (tests.get_supabase_user('client_1') ->> 'id')::UUID),
               ARRAY [
                   (tests.get_supabase_user('employee_1') ->> 'id')::UUID,
                   (tests.get_supabase_user('employee_2') ->> 'id')::UUID
                   ],
